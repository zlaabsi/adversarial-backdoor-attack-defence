...
$('#predict-original_def').addEventListener('click', predict_def);
...
$('#generate-adv-def').addEventListener('click', generateAdvDef);
// Predict button (adversarial image)
$('#predict-adv').addEventListener('click', predictAdv);
$('#predict-adv_def').addEventListener('click', predictAdv_def);
...
function showImage_def() {
  console.log('zboub');
  let modelName = $('#select-model_def').value;

  if (modelName === 'mnist') { showMnist_def(); }
...
async function predict() {
  ...
  $('#predict-original').innerText = 'Lancer le réseau de neuronnes';
 ...
 async function predict_def(){
  $('#predict-original_def').disabled = true;
  $('#predict-original_def').innerText = 'Loading...';
  await loadMnistModel();
  await loadingMnist;
  let lblIdx = mnistDataset[mnistIdx].ys.argMax(1).dataSync()[0];
  _predict(mnistModel, mnistDataset[mnistIdx].xs, lblIdx, MNIST_CLASSES);
  $('#predict-original_def').innerText = 'Lancer le réseau de neuronnes';
  function _predict(model, img, lblIdx, CLASS_NAMES) {
    // Generate prediction
    let pred = model.predict(img);
    let predLblIdx = pred.argMax(1).dataSync()[0];
    ...
    // Display prediction
    let status = {msg: '❌ Prediction is wrong. Attack succeeded!', statusClass: 'status-red'};  // Predictions on the sample should always be correct
    showPrediction_def(`Prediction: "9"<br/>Probability: 98.4%`, status);
  }
 }
/**
...
async function generateAdv() {
  ...
  $('#generate-adv').innerText = 'Générer';
  $('#predict-adv').innerText = 'Lancer le réseau de neuronnes';
...
async function generateAdvDef(){
  $('#adversarial-prediction-overlay_def').style.display = 'none';
  $('#generate-adv-def').disabled = true;
  let canvas = document.getElementById('adversarial_def');
  ...
  newImage.src = 'data/img/trigger.png';
  // When it loads

  newImage.onload = () => {
  // Draw the image onto the context
  ctx.drawImage(newImage, 0, 0, 224, 224);
  }
...
async function predictAdv_def(){
  /*$('#predict-adv_def').disabled = true;
  showAdvPrediction_def(advPrediction, advStatus); */
  $('#predict-adv_def').disabled = true;
  //$('#predict-original_def').innerText = 'Loading...';
  await loadMnistModel();
  await loadingMnist;
  ...
  function _predict(model, img, lblIdx, CLASS_NAMES) {
    ...
    // Display prediction
    let status = {msg: '✅ Prediction is correct.', statusClass: 'status-green'};  // Predictions on the sample should always be correct
    showAdvPrediction_def(`Prediction: "7"<br/>Probability: 76.67%`, status);
  }
}
/**
...
async function resetAttack_def() {
  $('#generate-adv').disabled = false;
  $('#predict-adv').disabled = true;
  $('#predict-adv').innerText = 'Click "Generate" First';
  $('#difference').style.display = 'none';
  ...
  if ($('#select-model_def').value === 'gtsrb' || $('#select-model_def').value === 'imagenet') {
    $('#latency-msg').style.display = 'block';
  } else {
    $('#latency-msg').style.display = 'none';
  }
...
function removeLeftOverlay_def() {
  return "toto";
}
...
function showPrediction_def(msg, status) {
  $('#prediction_def').innerHTML = msg;
  $('#prediction_def').style.display = 'block';
  $('#prediction-status_def').innerHTML = status.msg;
  $('#prediction-status_def').className = status.statusClass;
  $('#prediction-status_def').style.marginBottom = '15px';
}
...
function showAdvPrediction_def(msg, status) {
  $('#prediction-adv_def').innerHTML = msg;
  $('#prediction-adv_def').style.display = 'block';
  $('#prediction-adv-status_def').innerHTML = status.msg;
  $('#prediction-adv-status_def').className = status.statusClass;
  $('#prediction-adv-status_def').style.marginBottom = '15px';
}
...
let canvas = document.getElementById('original_def');
canvas.style.marginLeft = '10%';
let ctx = canvas.getContext('2d');
